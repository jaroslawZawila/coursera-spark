...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int ={
    val langLower = lang.toLowerCase
    rdd.aggregate(0)((number, wiki) => wiki.text.toLowerCase.split(" ").contains(langLower) match {case true => number + 1; case _ => number}
      , (x, y) => x + y)
  }
  ...
  def makeIndex(langs: List[String], rdd: RDD[WikipediaArticle]): RDD[(String, Iterable[WikipediaArticle])] =
    rdd.flatMap((wiki) => langs.filter(wiki.text.split(" ").contains(_)).map((_, wiki))).groupBy(_._1).mapValues(_.map(_._2))
